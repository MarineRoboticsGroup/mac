...
class GreedyEig:
    ...
    def subset(self, k, save_intermediate=False):
        ...
        chol_fiedler = CholeskyFiedlerSolver(self.L_odom, normalized=False, tol=1e-8, seed=np.random.RandomState(7))
        l2, fiedler_vec = chol_fiedler.find_fiedler_pair()
        ...
        for i in range(k):
            ...
            for j in range(len(self.weights)):
                ...
                cand_edge_ij = self.edge_list[j]
                cand_edge_weight = self.weights[j]
                cand_edge = Edge(cand_edge_ij[0], cand_edge_ij[1], cand_edge_weight)
                chol_fiedler.add_edge(cand_edge)
                l2, fiedler_vec = chol_fiedler.find_fiedler_pair()
                grad = self.grad_from_fiedler(fiedler_vec)
                chol_fiedler.remove_edge(cand_edge)
            ...
            chol_fiedler.add_edge(best_edge)
